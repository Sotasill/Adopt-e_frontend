...
const animalRegistrationSchema = Yup.object().shape({
  name: Yup.string()
    ...
    .max(100, "Кличка не должна превышать 100 символов")
    .matches(
      /^[A-Z][a-zA-Z\s]+( of [A-Za-z\s]+)?$/,
      "Кличка должна начинаться с большой буквы и может содержать только латинские буквы, пробелы и суффикс 'of CompanyName'"
    )
    .test(
      "name-unique",
